...
// 1) Write it as x = 1.NNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNNN000... in
// binary (1 <= x < 2, all 64 bits of n are used) 2) Calculate sqrt(x)
// = 1.0RRRRRRRRRRRRRRRRRRRRRRRRRRRRRRRR... (1 <= sqrt(x) < sqrt(2), so it will always start with
// "1.0" in binary) 3) Take 32 bits that come after "1.0" and return them as a 32-bit unsigned
...
// The reference implementation as it is now uses only unsigned int64 arithmetic, so it can't have
// undefined behavior It was tested once for all edge cases and confirmed correct
...
/*
VARIANT2_INTEGER_MATH_SQRT_FIXUP checks that "r" is an integer part of "sqrt(2^64 + sqrt_input) * 2
- 2^33" and adds or subtracts 1 if needed It's hard to understand how it works, so here is a full
calculation of formulas used in VARIANT2_INTEGER_MATH_SQRT_FIXUP

The following inequalities must hold for r if it's an integer part of "sqrt(2^64 + sqrt_input) * 2 -
2^33": 1) r <= sqrt(2^64 + sqrt_input) * 2 - 2^33 2) r + 1 > sqrt(2^64 + sqrt_input) * 2 - 2^33

We need to check them using only unsigned integer arithmetic to avoid rounding errors and undefined
behavior
...
If this inequality doesn't hold, then we must decrement r: IF "r2 + b^2/4 > sqrt_input" THEN r = r -
1
...
r2 + b = s*(s+b) + r*2^32 + b
The largest value s, b and r can have is s = 1779033703, b = 1, r = 3558067407 when sqrt_input =
2^64 - 1 r2 + b <= 1779033703*1779033704 + 3558067407*2^32 + 1 = 18446744068217447385 < 2^64
...
If this inequality doesn't hold, then we must decrement r: IF "r2 + 2^32 + (b+1)^2/4 <= sqrt_input -
s" THEN r = r - 1 b can be 0 or 1 If b is 0 then we need to compare "r2 + 2^32 + 1/4 <= sqrt_input -
s" which is equal to "r2 + 2^32 < sqrt_input - s" because all numbers here are integers If b is 1
then (b+1)^2/4 = 1, so we need to compare "r2 + 2^32 + 1 <= sqrt_input - s" which is also equal to
"r2 + 2^32 < sqrt_input - s"
-----------------------------------------------------------------------------------
...
The largest value s, b and r can have is s = 1779033703, b = 1, r = 3558067407 when sqrt_input =
2^64 - 1 r2 + b <= 1779033703*1779033704 + 3558067408*2^32 = 18446744072512414680 < 2^64

There will be no integer overflow when calculating "sqrt_input - s", i.e. "sqrt_input >= s" at all
times: s = trunc(r/2) = trunc(sqrt(2^64 + sqrt_input) - 2^32) < sqrt(2^64 + sqrt_input) - 2^32 + 1
...
This inequality is true if sqrt_input > 1 and it's easy to check that s = 0 if sqrt_input is 0 or 1,
so there will be no integer overflow
*/
